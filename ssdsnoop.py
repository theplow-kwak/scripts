...
class CaptureSSD:
    ...
    def fileopen(self):
        ...
        while True:
            ...
            if not os.path.exists(self.WorkloadFileName):
                break
            self.f_ext_no += 1
            continue
    ...
    def get_event(self, cpu, data, size):
        ...
        self.__values = [
            self.current_time,
            self.event.taskid.decode("utf-8"),
            self.event.major,
            self.event.minor,
            self.event.disk_name.decode("utf-8"),
            str(cmd_opcode.get(self.event.opcode, (self.event.opcode & 0xFF))),
            self.event.cmnd,
            self.event.slba,
            self.event.len,
            (self.event.latency_ns / 1000),
            self.event.ata_cmd,
        ]
