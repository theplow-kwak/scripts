...
set_disks()
{
    ...
    do
      if [[ -e $_IMG ]]; then
          ...
          if [[ -b $_IMG ]] && [[ $_IMG != *nvme* ]]; then _disk_type="scsi-hd"; else _disk_type="scsi-hd"; fi
          if [[ $_IMG == *.qcow2* ]]; then 
              DISKS+=" \
...
set_cdrom()
{
    ...
    do
        if [[ -e $_IMG ]]; then
            CDROMS+=" \
              -drive file=$_IMG,media=cdrom,readonly=on,if=$_IF,index=$_index,id=cdrom$_index"
